...
| Face lighting and material variation | `web/material-lighting.js`, `web/terrain-presentation.js` | Shared constants interpolated into the WebGL shader | Shared constants interpolated into the WGSL shader |
| Painterly water | `web/terrain-presentation.js` | Animated vertex displacement, shifting water tones, foam, Fresnel, and sun glint | Matching WGSL branches and constants |
...
| `npm run verify` (WSL) | Bend checks, proof, 78 regression tests, static build, and repository diff check | Passed |
...
| `tests/webgpu-terrain-presentation.test.mjs`, `tests/material-lighting.test.mjs` | Shared presentation constants, shader structure, atlas/material lighting, and surface effects | Contract tests pass |

In the local compositor used for this record, the adapter probe reported WebGPU support and the WGSL shader/pipeline validation completed with zero errors, but the visible-presentation probe rejected the canvas as blank and the runtime used WebGL. The WebGPU smoke therefore reports `skipped`/fallback without page errors and records the actual renderer as WebGL; shader validation is not visual presentation evidence. No WebGPU visual output, frame-time comparison, or pixel-identity claim is made. The six screenshots are visual evidence for the implemented composition; they are not a substitute for a target-hardware WebGPU capture.

...
- Procedurally synthesized WebGL texture atlas for blocks and entities: each tile is painted from an authored macro pattern and then shaded per texel by a material height field (cellular facets for stone, vertical blades for grass, fibres for wood, fine grain for sand), so surfaces carry real relief instead of flat 4x4 colour cells. Tiles are 64 texels in a 1024x1024 atlas, which is what a block face actually needs: at 30-60 screen pixels across, a 32-texel tile was being magnified to roughly one texel per pixel, so no amount of synthesis could read as detail. The fine tooth is a tiling value noise over a four-texel lattice rather than a per-texel hash, because white noise has energy at the Nyquist frequency and is therefore per-pixel noise once a tile is displayed 1:1, where no mip filtering can remove it. Face UVs and deterministic material variants are preserved, and the mip-chain certification now runs to level 5.
